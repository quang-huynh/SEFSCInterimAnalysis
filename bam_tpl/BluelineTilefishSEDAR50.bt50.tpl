//##--><>--><>--><>--><>--><>--><>--><>--><>--><>--><>--><>--><>--><>
//##
//##  SEDAR 50  Blueline Tilefish assessment model, 2017
//##  (Modified from: SEDAR 53  Red Grouper assessment model, Fall 2016)
...
// init_int endyr_selex_phase1_cH; // cH
// init_int endyr_selex_phase1_cL; // cL
init_int endyr_selex_phase1_GR; // GR
// ending years of selectivity block 2
// init_int endyr_selex_phase2_cH; // cH
...
//######################################################################################################
//## cH ######## INDEX ########
//## cH ## Starting and ending years of CPUE index
init_int styr_cH_cpue;                                             
init_int endyr_cH_cpue;
//## cH ## Observed index CPUE and CVs                                            
init_vector obs_cH_cpue(styr_cH_cpue,endyr_cH_cpue);
init_vector cH_cpue_cv(styr_cH_cpue,endyr_cH_cpue);

//## cH ######## LANDINGS ########
//## cH ## Starting and ending years of landings (bt50 landings+discards; includes cH and cO (commercial other))
init_int styr_cH_L;
init_int endyr_cH_L;
//## cH ## Observed landings (1000 lbs) and assumed CVs
init_vector obs_cH_L(styr_cH_L,endyr_cH_L);
init_vector cH_L_cv(styr_cH_L,endyr_cH_L);

//## cH ######## LENGTH COMPS ########
//## cH ## Number and vector of years of length compositions
init_int nyr_cH_lenc;
init_ivector yrs_cH_lenc(1,nyr_cH_lenc);
//## cH ## Sample size of length comp data (first row observed n.trips, second row n.fish)
init_vector nsamp_cH_lenc(1,nyr_cH_lenc);
init_vector nfish_cH_lenc(1,nyr_cH_lenc);
//## cH ## Observed length comps (3cm bins; proportions by year)
...
//######################################################################################################
//## cL ######## INDEX ########
//## cL ## Starting and ending years of CPUE index
init_int styr_cL_cpue;                                             
init_int endyr_cL_cpue;
//## cL ## Observed index CPUE and CVs                                            
init_vector obs_cL_cpue(styr_cL_cpue,endyr_cL_cpue);
init_vector cL_cpue_cv(styr_cL_cpue,endyr_cL_cpue);

//## cL ######## LANDINGS ########
//## cL ## Starting and ending years of landings (bt50 landings+discards)
init_int styr_cL_L;
init_int endyr_cL_L;
//## cL ## Observed landings (1000 lbs) and assumed CVs
init_vector obs_cL_L(styr_cL_L,endyr_cL_L);
init_vector cL_L_cv(styr_cL_L,endyr_cL_L);

//## cL ######## LENGTH COMPS ########
//## cL ## Number and vector of years of length compositions
init_int nyr_cL_lenc;
init_ivector yrs_cL_lenc(1,nyr_cL_lenc);
//## cL ## Sample size of length comp data (first row observed n.trips, second row n.fish)
init_vector nsamp_cL_lenc(1,nyr_cL_lenc);
init_vector nfish_cL_lenc(1,nyr_cL_lenc);

//## cL ## Observed length comps (3cm bins; proportions by year)
...
init_vector set_log_dm_cH_lc(1,7);  	// Dirichlet-multinomial overdispersion parameter (log-space): cH length comps
...
//######## Selectivity ########
init_vector set_selpar_A50_cH1(1,7);	// cH age at 0.5 selectivity
init_vector set_selpar_slope_cH1(1,7);	// cH slope of ascending limb
// init_vector set_selpar_A50_cH2(1,7);	// cH age at 0.5 selectivity (block 2)
// init_vector set_selpar_slope_cH2(1,7);	// cH slope of ascending limb (block 2)
// init_vector set_selpar_A502_cH2(1,7);	// cH L502 (block 2)
// init_vector set_selpar_slope2_cH2(1,7);	// cH slope of descending limb (block 2)
// init_vector set_selpar_A50_cH3(1,7);	// cH age at 0.5 selectivity (block 3)
// init_vector set_selpar_slope_cH3(1,7);	// cH slope of ascending limb (block 3)
...
//######## IndexCatchability ########
init_vector set_log_q_cH(1,7);      	// cH CPUE (log q)
...
init_vector set_log_avg_F_cH(1,7);		// cH log mean F
...
//## Dev vectors
//######################################################################################################
init_vector set_log_F_dev_cH(1,3); 		// cH F devs
...
init_number set_w_I_cH;         // cH index
init_number set_w_I_cL;         // cL index
init_number set_w_I_HB;         // HB index
init_number set_w_lc_cH;        // cH length comps
...
// time of year (as fraction) for spawning: end of May (from SEDAR 32)
...
init_number minSS_cH_lenc;	// cH len comps
...
//##################################################################################################
 ...
 LOCAL_CALCS
  ...
  // const double selpar_A502_cH2_LO=set_selpar_A502_cH2(2); const double selpar_A502_cH2_HI=set_selpar_A502_cH2(3); const double selpar_A502_cH2_PH=set_selpar_A502_cH2(4);
  // const double selpar_slope2_cH2_LO=set_selpar_slope2_cH2(2); const double selpar_slope2_cH2_HI=set_selpar_slope2_cH2(3); const double selpar_slope2_cH2_PH=set_selpar_slope2_cH2(4); 
...
// Set values of fixed parameters or set initial guess of estimated parameters
 ...
 //Population		
  ...
  // selpar_A502_cH2=set_selpar_A502_cH2(1);
...
FUNCTION get_selectivity
  ...
  sel_cL_block1=logistic(agebins, selpar_A50_cL1, selpar_slope_cL1);
  // sel_cL_block2=logistic(agebins, selpar_A50_cL2, selpar_slope_cL2);
  // sel_cL_block2=logistic_double(agebins, selpar_A50_cL2, selpar_slope_cL2, selpar_A502_cL2, selpar_slope2_cL2);  
  // sel_cL_block3=logistic(agebins, selpar_A50_cL3, selpar_slope_cL3);  
  ...
  // sel_HB_block3=sel_GR_block3; // Use GR selectivity for HB  
  ...
  //BLOCK 2 for selex  
  // for (iyear=(endyr_selex_phase1_cH+1); iyear<=endyr_selex_phase2_cH; iyear++)
   // {    
    // sel_cH(iyear)=sel_cH_block2;
   // }
  // BLOCK 3 for selex  
  // for (iyear=(endyr_selex_phase2_cH+1); iyear<=endyr; iyear++)
   // {    
  ...
  for (iyear=styr; iyear<=endyr; iyear++)
   {     
	sel_cL(iyear)=sel_cL_block1;
   }
  //BLOCK 2 for selex  
  // for (iyear=(endyr_selex_phase1_cL+1); iyear<=endyr_selex_phase2_cL; iyear++)
   // {    
	// sel_cL(iyear)=sel_cL_block2;
   // } 
  //BLOCK 3 for selex  
  // for (iyear=(endyr_selex_phase2_cL+1); iyear<=endyr; iyear++)
   // {    
	// sel_cL(iyear)=sel_cL_block3;
   // } 
  ...
  // BLOCK 2 for selex  
  for (iyear=(endyr_selex_phase1_GR+1); iyear<=endyr; iyear++)
   {    
    sel_HB(iyear)=sel_HB_block2;
    sel_GR(iyear)=sel_GR_block2;
   }
  //BLOCK 3 for selex  
  // for (iyear=(endyr_selex_phase2_GR+1); iyear<=endyr; iyear++)
   // {    
...
//Initialization
  ...
  F_init_denom=mfexp(log_avg_F_cH+log_F_dev_init_cH)+mfexp(log_avg_F_cL+log_F_dev_init_cL)+mfexp(log_avg_F_GR+log_F_dev_init_GR);
...
FUNCTION get_weighted_current 
  ...
  wgt_wgted_L_denom=F_cH_prop+F_cL_prop+F_GR_prop;  //+F_HB_prop
...
//switch here if var_rec_dev <=dzero 
  ...
  for(iyear=styr; iyear<=endyr; iyear++)
  {
        L_total_klb_yr(iyear)=pred_cH_L_klb(iyear)+pred_cL_L_klb(iyear)+pred_GR_L_klb(iyear);//+pred_HB_L_klb(iyear)
  ...
  L_total_num=L_cH_num+L_cL_num+L_GR_num;//+L_HB_num   //landings at age in number fish
  L_total_klb=L_cH_klb+L_cL_klb+L_GR_klb;//+L_HB_klb   //landings at age in klb whole weight
...
//pdf type 1=none, 2=lognormal, 3=normal, 4=beta 
  ...
  // f_priors+=neg_log_prior(selpar_A502_cH2,set_selpar_A502_cH2(5), set_selpar_A502_cH2(6), set_selpar_A502_cH2(7));
  // f_priors+=neg_log_prior(selpar_slope2_cH2,set_selpar_slope2_cH2(5), set_selpar_slope2_cH2(6), set_selpar_slope2_cH2(7));  
  ...
  f_priors+=neg_log_prior(log_q_cL,set_log_q_cL(5),set_log_q_cL(6),set_log_q_cL(7));  
...
REPORT_SECTION
  ...
  if (last_phase())  
  {
      ...
      //################################################################################################# 
       ...
       // selpar_A502_cH2_out(8)=selpar_A502_cH2; selpar_A502_cH2_out(1,7)=set_selpar_A502_cH2;
       // selpar_slope2_cH2_out(8)=selpar_slope2_cH2; selpar_slope2_cH2_out(1,7)=set_selpar_slope2_cH2;	   
       ...
       log_q_cL_out(8)=log_q_cL; log_q_cL_out(1,7)=set_log_q_cL;	   
   ...
   #include "bt50_make_Robject4.cxx"   // write the R-compatible report
