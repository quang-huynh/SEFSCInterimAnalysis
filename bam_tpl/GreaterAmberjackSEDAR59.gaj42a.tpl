...
////###Diving#################
//// Comm Diving  Landings (1000 lb gutted weight)
//init_int styr_cD_L;
//init_int endyr_cD_L;
...
//init_vector nfish_cD_lenc(1,nyr_cD_lenc);
//init_matrix obs_cD_lenc(1,nyr_cD_lenc,1,nlenbins);
////!!cout << "obs_cD_lenc" << endl;
////!!cout << obs_cD_lenc << endl;
//// Age Compositions 
...
////!!cout << "obs cD agec" << endl;
////!!cout << obs_cD_agec << endl;
...
// init_int nyr_cH_lenc_pool;
// init_ivector yrs_cH_lenc_pool(1,nyr_cH_lenc_pool);
// //# Annual sample size (nfish) of length comp data; used to weight years for pooling
// init_vector nfish_cH_lenc_pool(1,nyr_cH_lenc_pool);
...
//!!cout << "obs cH released" << endl;
//!!cout << obs_cH_released << endl;
//###################Headboat fleet ##########################################
// HB CPUE
...
// init_int nyr_GR_lenc_pool;
// init_ivector yrs_GR_lenc_pool(1,nyr_GR_lenc_pool);
// //# Annual sample size (nfish) of length comp data; used to weight years for pooling
// init_vector nfish_GR_lenc_pool(1,nyr_GR_lenc_pool);
...
//!!cout << "obs_GR_agec" << endl;
//!!cout << obs_GR_agec << endl;
//GR discard length comps - comes from the headboat fleet
init_int nyr_GR_D_lenc_pool;
init_ivector yrs_GR_D_lenc_pool(1,nyr_GR_D_lenc_pool);
//# Annual sample size (nfish) of length comp data; used to weight years for pooling
init_vector nfish_GR_D_lenc_pool(1,nyr_GR_D_lenc_pool);
...
//init_vector set_log_dm_cD_lc(1,7);    //Dirichlet-multinomial overdispersion parameter
...
init_vector set_log_dm_GR_D_lc(1,7);  //Dirichlet-multinomial overdispersion parameter
...
//nit_vector set_selpar_A50_cH2(1,7);
...
//init_vector set_selpar_A50_cD3(1,7);
...
//init_vector set_selpar_age1logit_GR_D(1,7);
...
//!!cout << "set_selpar_slope_GRD" << endl;
//!!cout << set_selpar_slope_GRD << endl;
//--index catchability-----------------------------------------------------------------------------------
...
//init_number set_w_lc_cD;        //weight for comm other len comps
//init_number set_w_lc_HB;        //weight for headboat len comps
init_number set_w_lc_GR;		//weight for the recreational length comps
init_number set_w_lc_GR_D;		//weight for the headboat discard length comps   
//init_number set_w_lc_cD;		//weight for the comm diving length comps  
...
//init_number set_w_ac_cD;		//weight for the comm diving age comps 
...
PARAMETER_SECTION
 ...
 LOCAL_CALCS
  ...
  //const double log_dm_cD_lc_LO=set_log_dm_cD_lc(2); const double log_dm_cD_lc_HI=set_log_dm_cD_lc(3); const double log_dm_cD_lc_PH=set_log_dm_cD_lc(4);
  ...
  //const double log_dm_cD_ac_LO=set_log_dm_cD_ac(2); const double log_dm_cD_ac_HI=set_log_dm_cD_ac(3); const double log_dm_cD_ac_PH=set_log_dm_cD_ac(4);
  ...
  // const double selpar_A50_cD3_LO=set_selpar_A50_cD3(2); const double selpar_A50_cD3_HI=set_selpar_A50_cD3(3); const double selpar_A50_cD3_PH=set_selpar_A50_cD3(4);
  ...
  //const double selpar_age1logit_GR_D_LO=set_selpar_age1logit_GR_D(2); const double selpar_age1logit_GR_D_HI=set_selpar_age1logit_GR_D(3); const double selpar_age1logit_GR_D_PH=set_selpar_age1logit_GR_D(4);
...
////--------------Growth--------------------------------------------------------------------------- 
 ...
 // matrix lenprob_cD(1,nages,1,nlenbins);     //distn of size at age in cD
  //matrix lenprob_HB(1,nages,1,nlenbins);     //distn of size at age in HB 
  matrix lenprob_GR_D(1,nages,1,nlenbins);   //distn of size at age in HB discards  
  matrix lenprob_GR(1,nages,1,nlenbins);     //distn of size at age in GR  
  //matrix lenprob_cD(1,nages,1,nlenbins);    //distn of size at age in CVT  
...
//----Predicted length and age compositions
  ...
  //matrix pred_cH_lenc_yr(1,nyr_cH_lenc_pool,1,nlenbins); //annual predicted length comps
  ...
  matrix pred_GR_D_lenc_yr(1,nyr_GR_D_lenc_pool,1,nlenbins); //annual predicted length comps
  //matrix pred_GR_lenc(1,nyr_GR_lenc,1,nlenbins);  
  //matrix pred_GR_lenc_yr(1,nyr_GR_lenc_pool,1,nlenbins); //annual predicted length comps
  ...
  //matrix pred_cD_agec(1,nyr_cD_agec,1,nages_agec);  
  //matrix pred_cD_agec_allages(1,nyr_cD_agec,1,nages);  
...
//Computed effective sample size for output (not used in fitting)
 ...
 // vector neff_HB_lenc_allyr(styr,endyr);
  vector neff_GR_D_lenc_allyr(styr,endyr);  
...
//---Stock-Recruit Function (Beverton-Holt, steepness parameterization)----------
  ...
  //init_bounded_number log_dm_cD_lc(log_dm_cD_lc_LO,log_dm_cD_lc_HI,log_dm_cD_lc_PH);
  ...
  //init_bounded_number log_dm_cD_ac(log_dm_cD_ac_LO,log_dm_cD_ac_HI,log_dm_cD_ac_PH);
  //vector log_dm_cH_lc_out(1,8);
  //vector log_dm_cD_lc_out(1,8);
...
//Commercial handline-------------------------------------------------
  ...
  //init_bounded_number selpar_A50_cD3(selpar_A50_cD3_LO,selpar_A50_cD3_HI,selpar_A50_cD3_PH);
...
//Discard selectivities
  ...
  //GR discards 
  //init_bounded_number selpar_age1logit_GR_D(selpar_age1logit_GR_D_LO,selpar_age1logit_GR_D_HI,selpar_age1logit_GR_D_PH);
  matrix sel_GR_D(styr,endyr,1,nages);
  vector sel_GR_D_vec(1,nages);
  //vector sel_GR_D_block2(1,nages);
  ...
  vector prob_belowsizelim_block2GR(1,nages);
  //vector prob_belowsizelim_block3(1,nages);
  number zscore_lsizelim2;
  number cprob_lsizelim2;
 //init_bounded_number selpar_age1logit_GR(selpar_age1logit_GR_LO,selpar_age1logit_GR_HI,selpar_age1logit_GR_PH);
 // number selpar_age1_GR_D; //age 1 value in arithmetic space, after logit backtransform 
 // vector selpar_age1logit_GR_D_out(1,8);
...
// Set values of fixed parameters or set initial guess of estimated parameters
  Dmort_cH=set_Dmort_cH; //Dmort_HB=set_Dmort_HB; 
 ...
 //Population		
  ...
  //log_dm_cD_lc=set_log_dm_cD_lc(1);
 ...
 //log_dm_cD_ac=set_log_dm_cD_ac(1);
  ...
  //selpar_A50_cD3=set_selpar_A50_cD3(1);
  ...
  //selpar_age1logit_GR_D=set_selpar_age1logit_GR_D(1);
...
//Used primarily for output in R object   
      ...
      //      nfish_cD_lenc_allyr(yrs_cD_lenc(iyear))=nfish_cD_lenc(iyear);}}			
      ...
      //       nfish_cD_agec_allyr(yrs_cD_agec(iyear))=nfish_cD_agec(iyear);}} 
...
//fill in F's, Catch matrices, and log rec dev with zero's
  ...
  //sel_GR_D_block2.initialize();  
  //sel_D_block3.initialize();
  sel_GR_D_vec.initialize();
  //sel_CVT_vec.initialize();
  prob_belowsizelim_block2.initialize();
  prob_belowsizelim_block2GR.initialize();
  ...
  //compute matrix of length at age, based on the normal distribution
   ...
   {len_cv(iage)=len_cv_val;
	...
	prob_belowsizelim_block2(iage)=	cprob_lsizelim1-cprob_lzero; //removes any probability mass below zero
        prob_belowsizelim_block2GR(iage)=cprob_lsizelim2-cprob_lzero;//for the general rec fleet
...
FUNCTION get_selectivity
  ...
  sel_cH_block1=logistic(agebins, selpar_A50_cH1, selpar_slope_cH1);  //KC set slope for block 1 = to that for block 2
  //sel_cH_block2=logistic(agebins, selpar_A50_cH2, selpar_slope_cH2);
  //sel_cH_block1=sel_cH_block2;
  
  //sel_cD_block2=logistic(agebins, selpar_A50_cD2, selpar_slope_cD2);
  // sel_cD_block2=logistic_double(agebins, selpar_A50_cD2, selpar_slope_cD2, selpar_A502_cD2,selpar_slope2_cD2);
  // sel_cD_block3=logistic_double(agebins, selpar_A50_cD3, selpar_slope_cD2, selpar_A502_cD2,selpar_slope2_cD2);
  ...
  //sel_GR_D_vec=logistic_double(agebins, selpar_A50_GRD, selpar_slope_GRD,selpar_A502_GRD, selpar_slope2_GRD);
  sel_GR_D_vec=logistic_exponential(agebins, selpar_A50_GRD, selpar_slope_GRD,selpar_A502_GRD, selpar_slope2_GRD);
  //selpar_age1_GR_D=mfexp(selpar_age1logit_GR_D)/(1.0+mfexp(selpar_age1logit_GR_D));
  //sel_GR_D_block2(1)=selpar_age1_GR_D; 
  //sel_GR_D_block2(2)=1.0; 
  //sel_GR_D_block2(3,nages)=prob_belowsizelim_block2GR(3,nages);
  
  sel_GR_block1=logistic(agebins, selpar_A50_GR1, selpar_slope_GR1); //KC set slope for block 1 = to that for block 2
  ...
  //selpar_age1_GR=mfexp(selpar_age1logit_GR)/(1.0+mfexp(selpar_age1logit_GR));
  //sel_GR_D_vec(1)=1.0;sel_GR_D_vec(2,nages)=prob_belowsizelim_block2GR(2,nages);
  ...
  for (iyear=styr; iyear<=endyr_selex_phase1; iyear++)
   {     
	...
	sel_GR_D(iyear)=sel_GR_D_vec;
	//sel_GR_D(iyear)=sel_GR_block1; //set to landings selex
    sel_GR(iyear)=sel_GR_block1;
	//sel_GR_D(iyear)=sel_GR_D_vec; //sel_GR_block1;
  ...
  //BLOCK 2 for selex.  36" size limit
   ...
   {   
	...
	sel_GR_D(iyear)=sel_GR_D_vec; //estimate discard selectivity from HB discards
	//sel_GR_D(iyear)=sel_GR_D_block2; 
    sel_GR(iyear)=sel_GR_block1;
	//sel_CVT(iyear)=sel_CVT_vec;
...
FUNCTION get_mortality
  ...
  for (iyear=styr; iyear<=endyr; iyear++) 
  {
	...
	F_GR_D(iyear)=sel_GR_D(iyear)*F_GR_D_out(iyear); 
...
//Initialization
  ...
  F_initial=sel_cH(styr)*mfexp(log_avg_F_cH+log_F_dev_init_cH)+
            //sel_cD(styr)*mfexp(log_avg_F_cD+log_F_dev_init_cD)+
...
FUNCTION get_landings_wgt
  ...
  {    
	...
	//L_cD_klb(iyear)=elem_prod(L_cD_num(iyear),wholewgt_cD_klb(iyear));     //in 1000 lb whole weight
...
//---Predicted CPUEs------------------------
 ...
 //HB  cpue
  ...
  for (iyear=styr_HB_cpue; iyear<=endyr_HB_cpue; iyear++)
  {   
      N_HB(iyear)=elem_prod(N_mdyr(iyear),sel_GR(iyear)); 
 ...
 // SERFS VID cpue
   ...
   {   
     N_VID(iyear)=N_mdyr(iyear);   //was elem_prod(N_mdyr(iyear),sel_VID(iyear));  if the selectivity is something other than 1 
...
FUNCTION get_length_comps
  ...
  //for (iyear=1;iyear<=nyr_cH_lenc_pool;iyear++)
  // {pred_cH_lenc_yr(iyear)=(L_cH_num(yrs_cH_lenc_pool(iyear))*lenprob_cH)/sum(L_cH_num(yrs_cH_lenc_pool(iyear)));}
  //
  //pred_cH_lenc.initialize();
  //for (iyear=1;iyear<=nyr_cH_lenc_pool;iyear++)
  //  {pred_cH_lenc(1) += nfish_cH_lenc_pool(iyear) * pred_cH_lenc_yr(iyear);}
  //pred_cH_lenc(1)=pred_cH_lenc(1)/sum(nfish_cH_lenc_pool);
  ...
  for (iyear=1;iyear<=nyr_GR_D_lenc_pool;iyear++)
   {pred_GR_D_lenc_yr(iyear)=(D_GR_num(yrs_GR_D_lenc_pool(iyear))*lenprob_GR_D)/sum(D_GR_num(yrs_GR_D_lenc_pool(iyear)));}
  
  pred_GR_D_lenc.initialize();
  for (iyear=1;iyear<=nyr_GR_D_lenc_pool;iyear++)
    {pred_GR_D_lenc(1) += nfish_GR_D_lenc_pool(iyear) * pred_GR_D_lenc_yr(iyear);}
  pred_GR_D_lenc(1)=pred_GR_D_lenc(1)/sum(nfish_GR_D_lenc_pool);
 ...
 //for (iyear=1;iyear<=nyr_GR_lenc_pool;iyear++)
 //  {pred_GR_lenc_yr(iyear)=(L_GR_num(yrs_GR_lenc_pool(iyear))*lenprob_GR)/sum(L_GR_num(yrs_GR_lenc_pool(iyear)));}
 // 
 // pred_GR_lenc.initialize();
 // for (iyear=1;iyear<=nyr_GR_lenc_pool;iyear++)
 //   {pred_GR_lenc(1) += nfish_GR_lenc_pool(iyear) * pred_GR_lenc_yr(iyear);}
 // pred_GR_lenc(1)=pred_GR_lenc(1)/sum(nfish_GR_lenc_pool);
...
//    pred_cD_agec_allages(iyear)=age_error*ErrorFree_cD_agec(iyear); 
...
//switch here if var_rec_dev <=dzero 
  ...
  for(iyear=styr; iyear<=endyr; iyear++)
  {
        L_total_klb_yr(iyear)=pred_cH_L_klb(iyear)+pred_GR_L_klb(iyear); //+pred_cD_L_klb(iyear)+pred_HB_L_klb(iyear)
        L_total_knum_yr(iyear)=pred_cH_L_knum(iyear)+pred_GR_L_knum(iyear); //+pred_cD_L_knum(iyear)+pred_HB_L_knum(iyear)
  ...
  L_total_num=L_cH_num+L_GR_num;   //landings at age in number fish  L_cD_num++L_HB_num
  L_total_klb=L_cH_klb+L_GR_klb;   //landings at age in klb whole weight  L_cD_klb++L_HB_klb
 
  D_total_num=(D_cH_num+D_GR_num);          //discards at age in number fish +D_HB_num
  D_total_klb=D_cH_klb+D_GR_klb;            //discards at age in klb whole weight +D_HB_klb
...
//---Length comps-------------------------------
  ...
  //f_GR_D_lenc=lk_logistic_normal(nsamp_GR_D_lenc, pred_GR_D_lenc, obs_GR_D_lenc, nyr_GR_D_lenc, double(nlenbins), minSS_GR_D_lenc);
  ...
  //cout << "headboat discards lenc" << f_GR_D_lenc << endl;
  //f_CVT_lenc
...
//pdf type 1=none, 2=lognormal, 3=normal, 4=beta 
  ...
  //f_priors+=neg_log_prior(selpar_A50_cD3,set_selpar_A50_cD3(5), set_selpar_A50_cD3(6), set_selpar_A50_cD3(7));
  ...
  //f_priors+=neg_log_prior(selpar_age1logit_GR_D,set_selpar_age1logit_GR_D(5), set_selpar_age1logit_GR_D(6), set_selpar_age1logit_GR_D(7));
  ...
  //f_priors+=neg_log_prior(log_dm_cD_lc,set_log_dm_cD_lc(5),set_log_dm_cD_lc(6),set_log_dm_cD_lc(7));
  ...
  //f_priors+=neg_log_prior(log_dm_cD_ac,set_log_dm_cD_ac(5),set_log_dm_cD_ac(6),set_log_dm_cD_ac(7));
...
REPORT_SECTION
  ...
  if (last_phase())  
  {
     ...
     ////sdnr_lc_cD=sdnr_multinomial(nyr_cD_lenc, lenbins, nsamp_cD_lenc, pred_cD_lenc, obs_cD_lenc, w_lc_cD);  	  
     ...
     ////sdnr_ac_cD=sdnr_multinomial(nyr_cD_agec, agebins_agec, nsamp_cD_agec, pred_cD_agec, obs_cD_agec, w_ac_cD);  
      ...
      //################################################################################################# 
	   ...
	   //log_dm_cD_lc_out(8)=log_dm_cD_lc; log_dm_cD_lc_out(1,7)=set_log_dm_cD_lc;
	   ...
	   //log_dm_cD_ac_out(8)=log_dm_cD_ac; log_dm_cD_ac_out(1,7)=set_log_dm_cD_ac;
       ...
       //selpar_A50_cD3_out(8)=selpar_A50_cD3; selpar_A50_cD3_out(1,7)=set_selpar_A50_cD3;
	   ...
	   //selpar_age1logit_GR_D_out(8)=selpar_age1logit_GR_D; selpar_age1logit_GR_D_out(1,7)=set_selpar_age1logit_GR_D;
   ...
   #include "gaj_make_Robject42a.cxx"   // write the R-compatible report
