...
plot_OM <- FALSE # Switch for plotting OM. It takes a while to plot them all
...
nsim <- 48 # The number of simulations (48 is the minimum number you can run in parallel)
...
maxF <- 0.8 # Maximum instantaneous fishing mortality rate that may be simulated for any given age class
...
par_cv <- c("TLinfinity"=0.2,"K"=0.2,"M"=0.2)
```
...
rec_AC <- A50 <- L50 <- setNames(rep(NA,length(bam_common_name_sub)),bam_common_name_sub)
...
for(rdat_name_i in bam_rdat_sub){
  ...
  slot(Stock_i,"maxage") <- length(a.series_i$age) # #Stock_otherValues[Name_i,"maxage"]
...
for(rdat_name_i in bam_rdat_sub[1]){
  Stock_name_i <- gsub("rdat","Stock",rdat_name_i)
  Stock_i <- get(Stock_name_i)
  rdat_i <- get(rdat_name_i)

  Name_i <- gsub("rdat_","",rdat_name_i)
  GenusSpecies_i <- Stock_otherValues[Name_i,"Species"]
  ...
  #a.series_i <- rdat_i$a.series
   ...
   tmax_i <- Stock_otherValues[Name_i,"maxage"]
   A50_i <- A50[[Name_i]]
  ...
  ## ..to generate correlated parameters   
   ...
   #par_sd <- apply(D.Gro_family_i[,c("TLinfinity","K","to","M")],2,function(x){sd(x,na.rm=TRUE)})
...
#   GenusSpecies_i <- Stock_otherValues[Name_i,"Species"]
#   genus_i <- unlist(str_extract_all(GenusSpecies_i,"^[A-Za-z]+"))
#   species_i <- unlist(str_extract_all(GenusSpecies_i,"[A-Za-z]+$"))
#   
#   # Get values from Fishbase
#   family_i <- D.taxa$Family[which(D.taxa$Genus==genus_i&D.taxa$Species==species_i)]
#   taxa_family_i <- as.data.frame(D.taxa[D.taxa$Family==family_i,])
...
#   Mean_pred_i <- Predict_i[[1]]$Mean_pred # Predictions from FishLife
#   
#   # fill in A50_i values if missing (e.g. for BluelineTilefish)
#   if(is.na(A50_i)){A50_i <- Mean_pred_i[["rho"]]}
# 
#    
#    # Generate correlated VB parameters
#    VBpar_mu <- c(Linf=Linf_i, K=K_i, t0=t0_i)
# 
#    
#    # D.Gro$sciname[grep("Lutjanus",D.Gro$sciname)]
#    
#    VBcv <-     c(Linf=0.1, K=0.1, t0=0.1)
#    # VBpar_cor <- rnorm_multi(
#    #   n = OM_i@nsim, 
#    #   mu = c(Linf=Linf_i, K=K_i, t0=t0_i),
#    #   sd = c(1, 5, 5),
#    #   r = c( 1,   0.2, -0.5, 
#    #          0.2, 1,    0.5, 
#    #         -0.5, 0.5,  1), 
#    #   varnames = c("Linf", "K", "t0"),
#    #   empirical = FALSE)
#    # 
#   
# }
...
OMToPlot <- local({
  a <- avail("OM")
  a[!a%in%c("testOM","OM_i")]
})
for(OM_name_i in OMToPlot){
