...
for(k in seq_along(OM_name)[2:3]) { ######### Loop over operating model
  ...
  for(i in seq_along(scenario)) { ######### Loop over scenario
    myOM <- readRDS(paste0("OM/", OM_name[k], "_OM.rds")) #NK# 2021-09-06 This is where this line should be
    ...
    # Save OM to object
    OM_name_ki <- paste0("OM_",OM_name[k], "_", scenario[i])
