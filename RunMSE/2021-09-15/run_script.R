...
seeds <- setNames(c(54, 86, 24),OM_name)
ntrials <- 500 # 300

DLMtool::setup(12) # Run in parallel over 12 cores


for(OM_name_k in OM_name[2]) { ######### Loop over operating model
  source('fn/iMP.R')
  # if(k == 3) source("fn/iMP_vs.R") ######## For Vermilion Snapper, selectivity is dome-shaped in SCA
  
  for(scenario_i in scenario[3]) { ######### Loop over scenario
    myOM <- readRDS(paste0("OM/", OM_name_k, ".rds")) #NK# 2021-09-06 This is where this line should be
    #myOM@seed <- 2
